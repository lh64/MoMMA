...
*/
aEff_Calculator::aEff_Calculator(ButcherTableau bt, vector<SimpleSpecies*>& sp, vector<Reaction*>& rxn)
		: bt(bt), sp(sp), rxn(rxn){
	if (MoMMA::debug)
		cout << "aEff_Calculator constructor called." << endl;
	// Add rxns
...
aEff_Calculator::aEff_Calculator(const aEff_Calculator& aCalc) : bt(aCalc.bt), a_eff(aCalc.a_eff),
		X_eff(aCalc.X_eff), rateSp(aCalc.rateSp), spInRxn(aCalc.spInRxn), stoich(aCalc.stoich), sp(aCalc.sp), rxn(aCalc.rxn){
	if (MoMMA::debug)
...
/*	if (this->upToDate){
		cout << "Error in aEff_Calculator::calc_aEff(): Must reset() calculator. Exiting." << endl;
		exit(1);
	}
	this->upToDate = true;
*/
	...
	for (unsigned int s=1;s < this->bt.size();s++){
		...
		for (unsigned int j=0;j < this->sp.size();j++){
			...
			for (unsigned int r=0;r < s;r++){
				if (fabs(this->bt.alpha[s][r]) > MoMMA::TOL){ // Skip the sum if alpha[s][r] is zero.
					double m_j = 0.0;
