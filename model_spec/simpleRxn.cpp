...
using namespace MoMMA;
	...
	SimpleRxn::SimpleRxn(double c, vector<SimpleSpecies*> reactants, vector<SimpleSpecies*> products, vector<int> stoichiometry){
		if (MoMMA::debug)
			cout << "SimpleRxn constructor called.\n";

		int nReactants = reactants.size();
		SimpleSpecies* rArray[nReactants];
		for (int i=0;i < nReactants;i++){
			rArray[i] = reactants.at(i);
		}

		int nProducts = products.size();
		SimpleSpecies* pArray[nProducts];
		for (int i=0;i < nProducts;i++){
			pArray[i] = products.at(i);
		}

		int stoich[stoichiometry.size()];
		for (unsigned int i=0;i < stoichiometry.size();i++){
			stoich[i] = stoichiometry.at(i);
		}

		this->construct(c,nReactants,rArray,nProducts,pArray,stoich);
	}
	...
	SimpleRxn::SimpleRxn(double c, int nReactants, SimpleSpecies **reactants, int nProducts,
		...
		if (MoMMA::debug)
			cout << "SimpleRxn constructor called.\n";
		this->construct(c,nReactants,reactants,nProducts,products,stoichiometry);
	}
	...
	void SimpleRxn::construct(double c, int nReactants, SimpleSpecies** reactants, int nProducts,
								SimpleSpecies** products, int* stoichiometry){
