...
RateHill::RateHill(double Vmax, double Kh, double h, vector<SimpleSpecies*> r, vector<int> rS) : Vmax(Vmax), Kh(Kh), h(h), rStoich(rS){
	...
	for (unsigned int i=0;i < r.size();i++){
		for (unsigned int j=i+1;j < r.size();j++){
			if (r[i] == r[j]){
				cout << "Error in RateHill constructor: Duplicate reactant species not allowed (r[" << i << "] = r[" << j << "] = ";
	...
	this->type += "Vmax=" + Util::toString(this->Vmax) + ",Kh=" + Util::toString(this->Kh) + ",h=" + Util::toString(this->h) + "}";
}
...
double RateHill::get_dRate_dX(unsigned int which, vector<double> X){
	...
	return max(0.0,dRate); // If dRate is negative, return 0.0 (just in case)
}
