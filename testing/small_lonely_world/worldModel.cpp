...
//	cout << endl;
	...
	for (int i=0;i<3;i++){
		...
		for (int j=0;j<2;j++){
			...
			reaction[madeRxns] = new SimpleRxn(k_Mot_f,nReactants,Cij0[i][j],nProducts,Cij1[i][j],stoich);
//			cout << "\t" + reaction[madeRxns]->name << endl;
			madeRxns++;
			reaction[madeRxns] = new SimpleRxn(k_Mot_r,nReactants,Cij1[i][j],nProducts,Cij0[i][j],stoich);
//			cout << "\t" + reaction[madeRxns]->name << endl;
			madeRxns++;
		}
	}
